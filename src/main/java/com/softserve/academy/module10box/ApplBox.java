package com.softserve.academy.module10box;

public class ApplBox {

	public static <T> T write(T t) {
		//Integer a = new Integer(1);
		System.out.println("type of t: " + t.getClass().getName() + "  t = " + t); // .toString();
		return t;
	}

	public static void main(String[] args) {
		/*-
		Box box = new Box();
		box.set(123); // int, -> Integer new Integer(123)
		// Code
		//
		box.set("123");
		//
		int i = (int) box.get() + 1; // Code Smell. Runtime Error
		System.out.println("i = " + i);
		*/
		/*-
		BoxWrapper box = new BoxWrapper();
		//box.set(123); // Compile Error.
		box.set("123");
		//
		//int i = (int) box.get(); // Compile Error.
		//int i = Integer.valueOf(box.get()); // try {} catch
		String i = box.get();
		System.out.println("i = " + (i + 11)); // 11 -> Integer -> toString();
		*/
		/*-
		BoxGen<String> box = new BoxGen<>();
		// box.set(123); // Compile Error.
		box.set("123"); // box.set( new String("123") );
		//
		// int i = (int) box.get(); // Compile Error.
		// int i = Integer.valueOf(box.get()); // try {} catch
		String i = box.get();
		System.out.println("i = " + (i + 11));
		//
		BoxGen<Integer> box2 = new BoxGen<>(); // = new BoxGen<Integer>();
		// box2.set("321"); // Compile Error.
		box2.set(321);  // box2.set( new Integer(321) ); // Autoboxig
		//
		int i2 = box2.get(); //  Unboxig
		// String i3 = (String) box2.get(); // Compile Error.
		System.out.println("i2 = " + (i2 + 11));
		//
//		write(box.get());
//		write(box2.get());
		*/
		// /*-
		BoxGen<A> box3 = new BoxGen<>(); // BoxGen box3 = new BoxGen<>(); Object by default
		box3.set(new A());
		write(box3.get());
		int i = write(345);
		System.out.println("i++ = " + ++i);
		// */
	}
}                  